            ...
            {['name', 'score', 'status'].map((field) => (
              <button
                ...
                className={`px-3 py-1 rounded text-xs font-semibold transition-colors flex items-center gap-1 ${
                  sortField === field ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
                }`}
              >
                {field.charAt(0).toUpperCase() + field.slice(1)}
                {sortField === field && <ArrowUpDown size={12} className={sortOrder === 'asc' ? 'rotate-180' : ''} />}
              </button>
            ))}
          </div>
        </div>
